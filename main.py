...
import torch
import numpy as np
from layers.CA_SUM.summarizer import CA_SUM
from layers.VASNet.vasnet_model import VASNet
from layers.SUM_GDA.summarizer import SUM_GDA
from os import listdir
from os.path import isfile, join
from configs import get_config
from data_loader import get_loader
from explanations import raw_attn, ent_attn, div_attn, grad_attn, grad_of_attn, input_norm_attn, input_norm_grad_attn
...
if __name__ == "__main__":
    ...
    visual_mask = config.visual_mask                    # black-frame, white-frame
    summarization_model = config.summarization_method   # CA_SUM, VASNet, SUM_GDA

    # Define the lists keeping the results for each data split
    att_disc_minus_all, ent_att_disc_minus_all, div_att_disc_minus_all, norm_att_disc_minus_all, grad_att_disc_minus_all, grad_of_att_disc_minus_all, norm_grad_att_disc_minus_all = [], [], [], [], [], [], []
    att_disc_plus_all, ent_att_disc_plus_all, div_att_disc_plus_all, norm_att_disc_plus_all, grad_att_disc_plus_all, grad_of_att_disc_plus_all, norm_grad_att_disc_plus_all = [], [], [], [], [], [], []
    att_sanity_test_all, ent_att_sanity_test_all, div_att_sanity_test_all, norm_att_sanity_test_all, grad_att_sanity_test_all, grad_of_att_sanity_test_all, norm_grad_att_sanity_test_all = [], [], [], [], [], [], []
    att_rank_corr_all, ent_att_rank_corr_all, div_att_rank_corr_all, norm_att_rank_corr_all, grad_att_rank_corr_all, grad_of_att_rank_corr_all, norm_grad_att_rank_corr_all = [], [], [], [], [], [], []

    for split_id in range(5):
        # Create model according to the configuration reported in the relevant paper
        if summarization_model == 'CA-SUM':
            model_path = f"...XAI-SUM/pretrained_models/CA_SUM/{dataset}/split{split_id}"
        ...
        elif summarization_model == 'VASNet':
            model_path = f"...XAI-SUM/pretrained_models/VASNet/{dataset}/split{split_id}"
            model_file = [f for f in listdir(model_path) if isfile(join(model_path, f))]
            trained_model = VASNet().to(device)
            trained_model.load_state_dict(torch.load(join(model_path, model_file[-1])))
        elif summarization_model == 'SUM-GDA':
            model_path = f"...XAI-SUM/pretrained_models/SUM_GDA/{dataset}/split{split_id}"
            model_file = [f for f in listdir(model_path) if isfile(join(model_path, f))]
            trained_model = SUM_GDA(input_size=1024, output_size=1024).to(device)
            trained_model.load_state_dict(torch.load(join(model_path, model_file[-1])))
        else:
            print("Wrong summarization method! Possible options: CA_SUM, VASNet, SUM_GDA")
            exit()
        ...
        ent_attn_eval = ExplanationEvalMeter(name="ent-attn", loader=video_loader, model=trained_model)
        div_attn_eval = ExplanationEvalMeter(name="div-attn", loader=video_loader, model=trained_model)
        ...
        for frame_features, fragments, video in video_loader:
            ...
            # Explanation mask: Entropy of Attention
            _, weights = ent_attn(trained_model, frame_features)
            ent_attn_eval.get_frames_weight(weights)
            ent_attn_eval.get_fragments_weight(fragments)
            ent_attn_eval.get_ranking(video)

            # Explanation mask: Diversity of Attention
            _, weights = div_attn(trained_model, frame_features)
            div_attn_eval.get_frames_weight(weights)
            div_attn_eval.get_fragments_weight(fragments)
            div_attn_eval.get_ranking(video)
        ...
        # Explanation mask: Entropy of Attention
        ent_att_disc_minus, ent_att_disc_plus, ent_att_sanity_test = get_measures(model=trained_model, loader=video_loader,
                                                                      ranked_fragments=ent_attn_eval.ranked_fragments,
                                                                      repl_method=repl_method,
                                                                      repl_fragments=repl_fragments,
                                                                      visual_token=visual_mask)

        # Explanation mask: Diversity of Attention
        div_att_disc_minus, div_att_disc_plus, div_att_sanity_test = get_measures(model=trained_model,
                                                                                  loader=video_loader,
                                                                                  ranked_fragments=div_attn_eval.ranked_fragments,
                                                                                  repl_method=repl_method,
                                                                                  repl_fragments=repl_fragments,
                                                                                  visual_token=visual_mask)
        ...
        if repl_fragments == "single":
            ...
            att_rank_correlation_values = att_rank_correlation_values[
              ~np.isnan(att_rank_correlation_values)]
            ...
            # Explanation mask: Entropy of Attention
            ent_att_rank_correlation_dict = rank_correlation(model=trained_model, loader=video_loader,
                                                         fragments_weight=ent_attn_eval.fragments_weight,
                                                         repl_method=repl_method, visual_token=visual_mask,
                                                         corr_method="spearman")

            # Fisher-z transformation for computing the average rank correlation
            ent_att_rank_correlation_values = np.array(list(ent_att_rank_correlation_dict.values()))
            # remove possible NaN values
            ent_att_rank_correlation_values = ent_att_rank_correlation_values[
                ~np.isnan(ent_att_rank_correlation_values)]
            z_values = np.arctanh(ent_att_rank_correlation_values)
            mean_z = np.mean(z_values)
            ent_att_rank_corr = np.tanh(mean_z)
            ent_att_rank_corr = np.round(ent_att_rank_corr, 3)

            # Explanation mask: Diversity of Attention
            div_att_rank_correlation_dict = rank_correlation(model=trained_model, loader=video_loader,
                                                         fragments_weight=div_attn_eval.fragments_weight,
                                                         repl_method=repl_method, visual_token=visual_mask,
                                                         corr_method="spearman")

            # Fisher-z transformation for computing the average rank correlation
            div_att_rank_correlation_values = np.array(list(div_att_rank_correlation_dict.values()))
            # remove possible NaN values
            div_att_rank_correlation_values = div_att_rank_correlation_values[
                ~np.isnan(div_att_rank_correlation_values)]
            z_values = np.arctanh(div_att_rank_correlation_values)
            mean_z = np.mean(z_values)
            div_att_rank_corr = np.tanh(mean_z)
            div_att_rank_corr = np.round(div_att_rank_corr, 3)
            ...
            # remove possible NaN values
            grad_att_rank_correlation_values = grad_att_rank_correlation_values[
               ~np.isnan(grad_att_rank_correlation_values)]
            ...
            # remove possible NaN values
            grad_of_att_rank_correlation_values = grad_of_att_rank_correlation_values[
               ~np.isnan(grad_of_att_rank_correlation_values)]
            ...
            # remove possible NaN values
            norm_att_rank_correlation_values = norm_att_rank_correlation_values[
               ~np.isnan(norm_att_rank_correlation_values)]
            ...
            # remove possible NaN values
            norm_grad_att_rank_correlation_values = norm_grad_att_rank_correlation_values[
               ~np.isnan(norm_grad_att_rank_correlation_values)]
        ...
        ent_att_disc_minus_all.append(ent_att_disc_minus)
        div_att_disc_minus_all.append(div_att_disc_minus)
        ...
        ent_att_disc_plus_all.append(ent_att_disc_plus)
        div_att_disc_plus_all.append(div_att_disc_plus)
        ...
        ent_att_sanity_test_all.append(ent_att_sanity_test)
        div_att_sanity_test_all.append(div_att_sanity_test)
        ...
        if repl_fragments == "single":
            ...
            ent_att_rank_corr_all.append(ent_att_rank_corr)
            div_att_rank_corr_all.append(div_att_rank_corr)
        ...
        table.field_names = ["Measure", "Raw Attention", "Input Norm Attention", "Grad Attention", "Grad of Attention",
                             "Input Norm Grad Attention", "Ent of Attention", "Div of Attention"]
        table.add_row(["Discoverability-", att_disc_minus, norm_att_disc_minus, grad_att_disc_minus,
                       grad_of_att_disc_minus, norm_grad_att_disc_minus, ent_att_disc_minus, div_att_disc_minus])
        table.add_row(["Discoverability+", att_disc_plus, norm_att_disc_plus, grad_att_disc_plus,
                       grad_of_att_disc_plus, norm_grad_att_disc_plus, ent_att_disc_plus, div_att_disc_plus])
        table.add_row(
            ["Sanity Violation", att_sanity_test, norm_att_sanity_test, grad_att_sanity_test, grad_of_att_sanity_test,
             norm_grad_att_sanity_test, ent_att_sanity_test, div_att_sanity_test])
        if repl_fragments == "single":
            table.add_row(["Rank Correlation", att_rank_corr, norm_att_rank_corr, grad_att_rank_corr, grad_of_att_rank_corr,
                           norm_grad_att_rank_corr, ent_att_rank_corr, div_att_rank_corr])
        print(table)

    # Compute overall results (averaged values across splits)
    att_disc_minus_overall = np.round(np.array(att_disc_minus_all).mean(), 3)
    ent_att_disc_minus_overall = np.round(np.array(ent_att_disc_minus_all).mean(), 3)
    div_att_disc_minus_overall = np.round(np.array(div_att_disc_minus_all).mean(), 3)
    ...
    ent_att_disc_plus_overall = np.round(np.array(ent_att_disc_plus_all).mean(), 3)
    div_att_disc_plus_overall = np.round(np.array(div_att_disc_plus_all).mean(), 3)
    ...
    ent_att_sanity_test_overall = np.round(np.array(ent_att_sanity_test_all).mean(), 3)
    div_att_sanity_test_overall = np.round(np.array(div_att_sanity_test_all).mean(), 3)
    ...
    if repl_fragments == "single":
        ...
        z_values = np.arctanh(np.array(ent_att_rank_corr_all))
        mean_z = np.mean(z_values)
        ent_att_rank_corr_all_mean = np.tanh(mean_z)
        ent_att_rank_corr_overall = np.round(ent_att_rank_corr_all_mean, 3)

        z_values = np.arctanh(np.array(div_att_rank_corr_all))
        mean_z = np.mean(z_values)
        div_att_rank_corr_all_mean = np.tanh(mean_z)
        div_att_rank_corr_overall = np.round(div_att_rank_corr_all_mean, 3)
    ...
    table.add_row(["Discoverability-", att_disc_minus_overall, norm_att_disc_minus_overall, grad_att_disc_minus_overall,
                   grad_of_att_disc_minus_overall, norm_grad_att_disc_minus_overall, ent_att_disc_minus_overall, div_att_disc_minus_overall])
    table.add_row(["Discoverability+", att_disc_plus_overall, norm_att_disc_plus_overall, grad_att_disc_plus_overall,
                   grad_of_att_disc_plus_overall, norm_grad_att_disc_plus_overall, ent_att_disc_plus_overall, div_att_disc_plus_overall])
    table.add_row(
        ["Sanity Violation", att_sanity_test_overall, norm_att_sanity_test_overall, grad_att_sanity_test_overall,
         grad_of_att_sanity_test_overall, norm_grad_att_sanity_test_overall, ent_att_sanity_test_overall, div_att_sanity_test_overall])
    if repl_fragments == "single":
        table.add_row(
            ["Rank Correlation", att_rank_corr_overall, norm_att_rank_corr_overall, grad_att_rank_corr_overall,
             grad_of_att_rank_corr_overall, norm_grad_att_rank_corr_overall, ent_att_rank_corr_overall, div_att_rank_corr_overall])
    print(table)
