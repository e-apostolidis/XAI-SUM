...
def sanity_violation_test(model, loader, fragments_weight, repl_method, visual_token):
    ...
    for frame_features, fragments, video in loader:
        baseline, _, _, _ = inference(model=model, features=frame_features)  # The baseline scores
        ...
        if repl_method == "slice-out":
            ...
            prediction, _, _, _ = inference(model, features_no_fragm)  # The newly predicted imp. scores
            fragments = fragments[:-1]
        elif repl_method == "attention-mask":
            prediction, _, _, _ = inference(model, frame_features, mask_attn=True, fragments=[fragment])
    ...
    for frame_features, fragments, video in loader:
        ...
        for fragment in fragments:
            if repl_method == "slice-out":                                     # Omit the shot entirely
                ...
                features_no_fragm = slice_out(frame_features, [fragment])      # Remove the features of the fragment(s)
                score_no_fragm, _, _, _ = inference(model, features_no_fragm)  # The newly predicted imp. scores

                fragments_comprehension.append(comprehensiveness(score_no_fragm, base_score_no_fragm.numpy()))
            elif repl_method == "attention-mask":
                base_score = baseline.copy()
                score_masked_fragm, _, _, _ = inference(model, frame_features, mask_attn=True, fragments=[fragment])
                ...
                base_score = baseline.copy()
                features_random_fragm = random_replace(frame_features, [fragment], size=512)
                score_random_fragm, _, _, _ = inference(model, features_random_fragm)
...
def get_measures(model, loader, ranked_fragments, repl_method, repl_fragments, visual_token) -> Tuple[float, float, float]:
    ...
    for frame_features, fragments, video in loader:
        ...
        for _iter in iterations:
            ...
            if repl_method == "slice-out":  # Omit the shot entirely
                ...
                score_no_top, _, _, _ = inference(model, features_no_top)            # The newly predicted imp. scores
                ...
                score_no_less, _, _, _ = inference(model, features_no_less)          # The newly predicted imp. scores
                ...
                score_masked_top, _, _, _ = inference(model, frame_features,
                                                      mask_attn=True, fragments=top_scored_fragments)
                score_masked_less, _, _, _ = inference(model, frame_features,
                ...
                score_masked_top, _, _, _ = inference(model, features_masked_top)

                features_masked_less = input_msk(frame_features, less_scored_fragments, token=visual_mask)
                score_masked_less, _, _, _ = inference(model, features_masked_less)
                ...
                score_random_top, _, _, _ = inference(model, features_random_top)

                features_random_less = random_replace(frame_features, less_scored_fragments, size=512)
                score_random_less, _, _, _ = inference(model, features_random_less)
    ...
    # sanity = 100 - np.round(np.array(sanity).mean() * 100, 2)
